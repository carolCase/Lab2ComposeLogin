    ...
    fun onEvent(event:SignUpUIEvent){
     ...
     private fun errorHandlingWithRules(){
        ...
        signUpUIState.value = signUpUIState.value.copy(
         ...
         //this variable checks that input is right and makes the button register
   ...
   private fun createUserFirebase(email: String, password : String){
        FirebaseAuth
            ...
            .addOnCompleteListener{
                 if (it.isSuccessful){
                   NavigationRouter.navigateTo(HomeScreen)
                 }
            }
...
fun logout(){
    ...
    firebaseAuth.signOut() //signs the current user out of the Firebase authentication system.
    val authStateListener = AuthStateListener{
        if(it.currentUser == null){
           NavigationRouter.navigateTo(LoginScreen)
        }
    }
